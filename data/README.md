...
保留原有规模：12位虚构老人、每人30天记录、每人2条病历。只有下面三张CSV表，它们用 `patient_id` 关联；另有三份供上传测试的中文模拟病历PDF，放在 `测试数据/`：[SYN-001](测试数据/SYN-001_模拟病历.pdf)、[SYN-003](测试数据/SYN-003_模拟病历.pdf)、[SYN-006](测试数据/SYN-006_模拟病历.pdf)。所有内容都是模拟的。
